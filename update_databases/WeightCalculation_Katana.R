...
drugs <- read.csv("/srv/scratch/z3526914/DrugRepo/Data/drug links.csv")
drugs <- subset(drugs, Drug.Type == "SmallMoleculeDrug")

# Step 1: Read files

sc.m <- as.matrix(read.csv("/srv/scratch/z3526914/DrugRepo/Data/chem_similarity.csv", row.names = 1))
diag(sc.m) = 0
...
sc.w <- sapply(sc.m, function(x) sapply(x, getWeight, sc.q))

save(sc.w,file="sc.w.rda")
