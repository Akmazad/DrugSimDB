...
library(dplyr)
library(tidyr)
data <- read_xml("/short/yr31/aa7970/azData/DD/Data/full database.xml")
drugs <- xml_find_all(data, "//d1:drugbank/d1:drug")

DB_ID <- xml_find_first(drugs, ".//d1:drugbank-id") %>%
  xml_text() %>%
  tbl_df() %>%
  rename("DB_ID" = value)
...
dg <- xml_find_first(drugs, ".//d1:groups") %>%
  lapply(function(x){xml_find_all(x,".//d1:group") %>% xml_text() %>% paste(collapse = ',')}) %>%
  as.character() %>%
  tbl_df() %>%
  rename("Groups" = value)
print("dg is processed")
...
# Write tbl on disk
write.csv(as.data.frame(tbl),"DB_full_meta_data.csv",quote = T,row.names = F)